...
CAPSLIST *ScGetCapsEx(RPC *rpc)
{
	...
	if (ScGetCaps(rpc, t) != ERR_NO_ERROR)
	{
		...
		if (is_bridge == false)
		{
			AddCapsInt(t, "i_max_hubs", 409600);
			AddCapsInt(t, "i_max_sessions", 409600);

			if (info.ServerType != SERVER_TYPE_FARM_MEMBER)
			{
				AddCapsInt(t, "i_max_users_per_hub", 100000);
				AddCapsInt(t, "i_max_groups_per_hub", 100000);
				AddCapsInt(t, "i_max_access_lists", 409600);
			}
			else
			{
		...
		AddCapsInt(t, "i_max_mac_tables", 100000);
		AddCapsInt(t, "i_max_ip_tables", 100000);

		if (info.ServerType == SERVER_TYPE_STANDALONE)
		{
			AddCapsBool(t, "b_support_securenat", (build >= 3600) ? true : false);
			AddCapsInt(t, "i_max_secnat_tables", 409600);
		}
		else
		{
