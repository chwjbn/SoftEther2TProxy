...
#run network
ip tuntap add tap_vpn mode tap
ifconfig tap_vpn 172.16.0.1 netmask 255.255.0.0 promisc
...
echo \# TPROXY_TO=[${TPROXY_TO}]

nft add table ip vernus
nft add chain ip vernus prerouting { type filter hook prerouting priority 0\; }
nft add rule vernus prerouting iif "tap_vpn" meta l4proto {tcp} mark set 8 tproxy to $TPROXY_TO accept

ip rule add fwmark 8 table 88 
